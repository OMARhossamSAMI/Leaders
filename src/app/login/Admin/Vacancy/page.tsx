...
interface Application {
  _id: string;
  data?: Record<string, any>; // data can be undefined
  createdAt: string;
}
...
export default function ApplicationsPage() {
  ...
  const handleUpdate = async (id: string) => {
    await axios.patch(`http://localhost:3000/vacancy/${id}`, { data: editData });
    setEditingId(null);
    setExpandedId(null);
            ...
            <div className="card shadow-sm border-0 application-card">
              <div className="card-body">
                <h5 className="card-title">
                  {app.data?.full_name || "Applicant"}
                </h5>
                <p className="card-text">
                  <strong>Position:</strong> {app.data?.position || "N/A"}<br />
                  <strong>Date:</strong>{" "}
